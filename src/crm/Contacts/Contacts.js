    ...
    let assigned_to = (user.assigned_to.length !== 0) ?         
      ...
      return(
        <tr style={{textAlign:'center'}}>
          ...
          <td><a href="/#" data-toggle="modal" data-target={`#exampleModalCenter_contact${user.id}`}>{user.first_name +' ' + user.last_name}</a></td>
          {assigned_to}          
          <td>{(user.address.city) ? user.address.city : 'Not Specified'}</td>
          <td>{(user.address.state) ? user.address.state : 'Not Specified'}</td>
          <td>{user.created_on_arrow}</td>
